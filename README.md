...
3. **Aggregation by median.** `llm_aggregator.py` groups the three classifiers' verdicts for the same route and takes the ordinal median of their `benign_level` (Low < Medium < High) as the final verdict, unions their contributing factors, and reports how many of the three agreed as `aggregation_confidence`.
